import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

import 'app_colors.dart';

class AppTextStyles {
  static final TextStyle fonte = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 32,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle title = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 50,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle titleBlue = GoogleFonts.ramabhadra(
    color: AppColors.blue,
    fontSize: 26,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle subtitleBlue = GoogleFonts.ramabhadra(
    color: AppColors.blue,
    fontSize: 18,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle botoesListaVoo = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 14,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle listraListaVoo = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 23,
     // fontWeight: FontWeight.w100,
  );

  static final TextStyle dropDown = GoogleFonts.ramabhadra(
    color: AppColors.blue,
    fontSize: 15,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle dropDownTitle = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 16  ,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle subtitle = GoogleFonts.ramabhadra(
    fontSize: 12  ,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle detalhes = GoogleFonts.ramabhadra(
    fontSize: 15  ,
    color: AppColors.blue,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle detalhesTitle = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 15  ,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle button = GoogleFonts.ramabhadra(
    color: AppColors.blue,
    fontSize: 17,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle fonteLista = GoogleFonts.ramabhadra(
    color: AppColors.blue,
    fontSize: 16,
    // fontWeight: FontWeight.w600,
  );
  static final TextStyle body = GoogleFonts.ramabhadra(
    color: AppColors.white,
    fontSize: 13,
    fontWeight: FontWeight.normal,
  );
}